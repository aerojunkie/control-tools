...
function draw() {
  ...
  textSize(25);
  text('With T = 2 seconds', 530, 750);
  textSize(40);
  text('Mapping to z-plane with bilinear transform', 280, 50);
  nudge = 50;
  drop = 20;
  text('z = ', 480 + nudge, 115 + drop);
  text('1 + sT/2', 550 + nudge, 93 + drop);
  text('1 - sT/2', 550 + nudge, 137 + drop);
  strokeWeight(3);
  stroke(255);
  line(550 + nudge, 100 + drop, 700 + nudge, 100 + drop)
  stroke(0);
  strokeWeight(1);
...
function z_target() {
  ...
  this.show = function() {
    ...
    text(warpTr.substring(0, 5) + ', ' + warpTi.substring(0, 5), this.zr*zscale + zx0 + 5, this.zi*zscale + zy0 - 5);
...
function getMapExp(s_spot) {
  this.sr = s_spot[0];
  this.si = s_spot[1];
  this.zr = Math.exp(this.sr) * Math.cos(this.si);
  this.zi = Math.exp(this.sr) * Math.sin(this.si);
  return [this.zr, this.zi];
}
